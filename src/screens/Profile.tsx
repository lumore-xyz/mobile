...
const ProfileScreen: React.FC<ProfileScreenProps> = ({ profileUserId }) => {
  ...
  const { user } = useUser(targetUserId) as any;
  const { posts, isLoading: isPostsLoading } = useUserPosts(targetUserId);
  const { userPrefrence } = useUserPrefrence(targetUserId);
      ...
      >
        <View className="flex-1 w-full">
                ...
                <Text className="text-2xl font-semibold text-ui-shade">
                  ...
                  user.nickname ? (
                    <>
                      ...
                      <Text className="text-ui-shade/60">
                        {" "}
                        ({user.nickname})
                      </Text>
                    </>
            ...
            <View className="mt-4 rounded-2xl bg-white border border-ui-shade/10 p-4">
              ...
              {!user?.isVerified ? (
                <Pressable
                  ...
                  disabled={isStartingVerification || user?.verificationStatus === "pending"}
                  className={`mt-3 rounded-xl border border-ui-highlight/30 bg-ui-highlight/10 px-4 py-3 ${
                    isStartingVerification || user?.verificationStatus === "pending"
                      ? "opacity-70"
                      : ""
                  }`}
                >
          ...
          <View className="mt-4 rounded-2xl bg-white border border-ui-shade/10 p-4">
            ...
            {isThisOrThatLoading ? (
              <Text className="text-xs text-ui-shade mt-2">Loading answers...</Text>
          ...
          <View className="mt-4">
            ...
            {isPostsLoading ? (
              <Text className="text-xs text-ui-shade">Loading posts...</Text>
            ) : posts?.length ? (
...
const PromptPost = ({ post }: { post: any }) => {
  ...
  return (
    <View className="p-4">
      <Text className="text-sm text-ui-shade/70">{promptText}</Text>
      <Text className="text-base font-semibold mt-2">{answer}</Text>
    </View>
  );
};
...
const TextPost = ({ post }: { post: any }) => (
  <View className="p-4">
    <Text className="text-sm text-ui-shade whitespace-pre-wrap">
      {post?.content?.text || ""}
    </Text>
  </View>
);
